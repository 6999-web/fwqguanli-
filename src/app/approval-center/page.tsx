...
export default function ApprovalCenterPage() {
  ...
  function getDraft(approval: any): WorkspaceSpecDraft {
    return drafts[approval.id] ?? {
      ...defaultDraft,
      targetServerId: approval.payload?.serverId ?? "",
    ...
    role === "ADMIN" && approval.status === "PENDING" ? (
      <div key={approval.id} className="space-y-3">
        {approval.type === "WORKSPACE_ACCESS" || approval.type === "SERVER_USAGE" ? (
          <div className="grid gap-2 sm:grid-cols-2">
            <select
              ...
              value={getDraft(approval).targetServerId}
              onChange={(event) => updateDraft(approval.id, { targetServerId: event.target.value })}
            >
            ...
            <input
              className="rounded bg-[#031224] px-3 py-2 text-xs text-slate-200"
              type="date"
              value={getDraft(approval).dueAt}
              onChange={(event) => updateDraft(approval.id, { dueAt: event.target.value })}
            />
            <input
              className="rounded bg-[#031224] px-3 py-2 text-xs text-slate-200"
              placeholder="CPU"
              value={getDraft(approval).cpuLimit}
              onChange={(event) => updateDraft(approval.id, { cpuLimit: event.target.value })}
            />
            <input
              className="rounded bg-[#031224] px-3 py-2 text-xs text-slate-200"
              placeholder="内存 MB"
              value={getDraft(approval).memoryLimitMb}
              onChange={(event) => updateDraft(approval.id, { memoryLimitMb: event.target.value })}
            />
            <input
              className="rounded bg-[#031224] px-3 py-2 text-xs text-slate-200"
              placeholder="磁盘 GB"
              value={getDraft(approval).diskLimitGb}
              onChange={(event) => updateDraft(approval.id, { diskLimitGb: event.target.value })}
            />
            <input
              className="rounded bg-[#031224] px-3 py-2 text-xs text-slate-200"
              placeholder="GPU"
              value={getDraft(approval).gpuLimit}
              onChange={(event) => updateDraft(approval.id, { gpuLimit: event.target.value })}
            />
            <input
              className="rounded bg-[#031224] px-3 py-2 text-xs text-slate-200"
              placeholder="SSH 端口"
              value={getDraft(approval).sshPort}
              onChange={(event) => updateDraft(approval.id, { sshPort: event.target.value })}
            />
            <input
              className="rounded bg-[#031224] px-3 py-2 text-xs text-slate-200"
              placeholder="业务端口起始"
              value={getDraft(approval).hostPortStart}
              onChange={(event) => updateDraft(approval.id, { hostPortStart: event.target.value })}
            />
            <input
              className="rounded bg-[#031224] px-3 py-2 text-xs text-slate-200"
              placeholder="业务端口结束"
              value={getDraft(approval).hostPortEnd}
              onChange={(event) => updateDraft(approval.id, { hostPortEnd: event.target.value })}
            />
            <input
              className="rounded bg-[#031224] px-3 py-2 text-xs text-slate-200"
              placeholder="端口数量"
              value={getDraft(approval).requestedPortCount}
              onChange={(event) => updateDraft(approval.id, { requestedPortCount: event.target.value })}
            />
            <input
              className="rounded bg-[#031224] px-3 py-2 text-xs text-slate-200"
              placeholder="宽限天数"
        ...
        <div className="flex gap-2">
          <button className="rounded bg-cyan-500/10 px-3 py-1 text-cyan-100" onClick={() => decide(approval, true)}>
            通过
          </button>
          <button className="rounded bg-rose-500/10 px-3 py-1 text-rose-200" onClick={() => decide(approval, false)}>
            驳回
          </button>
        </div>
      </div>
